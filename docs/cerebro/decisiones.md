...
- **Corrimiento de columnas (encabezado vs dato):** primero se quitó la primera columna *vacía* del ícono de alerta (se movió dentro de la celda Fecha con hueco de 15px) — **no bastó**, el corrimiento persistía. Causa real: el **CSS bespoke `.facturaTable`** desincronizaba el ancho de columnas entre `thead`/`tbody`. **Fix definitivo (2026-06-20):** la tabla pasó a las clases probadas del DS **`.ds-table`** (mismo patrón que `solicitudes-transporte`/`usuarios`/`auditoria`, que alinean bien): `<table className="ds-table" minWidth:820>` dentro de `<div overflowX:auto>` dentro de `.tablePanel`; filas con `is-selected` + `--row-color`; `th.sortable`. Se borró el CSS bespoke `.facturaTable`/`.row`/`.tableScroll`/`.sortable`. Tabla final: 5 columnas alineadas.
