...
import { Boxes, Clock, Truck, CheckCircle2 } from '@lucide/vue'

const props = defineProps<{ counts: { total: number; pendienteArea2: number; listaTransporte: number; completada: number } }>()
const emit = defineEmits<{ (e: 'filter', key: string): void }>()

const k = computed(() => props.counts)

const cards = computed(() => [
  { key: 'total', label: 'Total integraciones', value: k.value.total, tone: 'var(--ink)', icon: Boxes, filter: '', hint: 'registradas' },
  { key: 'pend', label: 'Pendientes Área 2', value: k.value.pendienteArea2, tone: 'var(--u-aviso)', icon: Clock, filter: 'PENDIENTE_AREA2', hint: 'por completar' },
  { key: 'lista', label: 'Listas para transporte', value: k.value.listaTransporte, tone: 'var(--info)', icon: Truck, filter: 'LISTA_TRANSPORTE', hint: 'esperando recepción' },
  { key: 'completada', label: 'Completadas', value: k.value.completada, tone: 'var(--u-ok)', icon: CheckCircle2, filter: 'COMPLETADA', hint: 'listas' },
])
</script>
