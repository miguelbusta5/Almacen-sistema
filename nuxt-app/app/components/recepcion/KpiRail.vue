...
import { Container, Timer, Boxes, FileWarning } from '@lucide/vue'
import { fmtTiempoRecepcion, type RecepcionConteos } from '~/utils/recepcion'

const props = defineProps<{ counts: RecepcionConteos }>()
const emit = defineEmits<{ (e: 'filter', key: string): void }>()

const cards = computed(() => [
  {
    key: 'hoy', label: 'Contenedores hoy', value: props.counts.recepcionesHoy,
    tone: 'var(--brand)', icon: Container, filter: '', hint: 'del dia',
  },
  {
    key: 'curso', label: 'En curso', value: props.counts.enCurso,
    tone: 'var(--info)', icon: Timer, filter: 'en-curso',
    hint: props.counts.promedioSeg === null
      ? 'sin cronometrar'
      : `prom. ${fmtTiempoRecepcion(props.counts.promedioSeg)}`,
  },
  {
    key: 'uds', label: 'Unidades hoy', value: props.counts.unidadesHoy,
    tone: 'var(--ink)', icon: Boxes, filter: '',
    hint: `${props.counts.cajasHoy} cajas · ${props.counts.estibasHoy} estibas`,
  },
  {
    key: 'nov', label: 'Con novedad', value: props.counts.conNovedad,
    tone: 'var(--u-aviso)', icon: FileWarning, filter: 'novedad',
    hint: 'faltantes, averias...',
  },
])
</script>
