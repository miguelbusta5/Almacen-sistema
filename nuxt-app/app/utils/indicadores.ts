...
export interface RespuestaIndicadores {
  ...
  carga?: CargaPersona[]
  /** Tareas, pendientes y movimientos de cada persona por dia de turno. */
  cierresDiarios?: CierresDiaPersona[]
  /** Lo del equipo por dia, cada registro una vez (no es la suma de las personas). */
  equipoDiario?: Array<{ dia: string; tareas: number; pendientes: number; movimientos: number; total: number }>
...
export interface CierresDiaPersona {
  ...
  movimientos: number
  /** tareas + pendientes + movimientos: todo en lo que participo. */
  total: number
  /** De ese total, las que hizo con otra persona (tambien le cuentan a ella). */
  compartidas: number
}
