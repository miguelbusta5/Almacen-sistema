...
// Nota: clientes con bundle viejo aún pueden enviar `tieneParte2` — zod
// ignora claves desconocidas, así que quitarlo del schema no rompe nada.
// La repetición de cajas MUEBLES ahora se detecta sola por multiplicidad
// (cuántas veces se registró el código al crear el pedido).
...
export default defineEventHandler(async (event) => {
  ...
  // entonces sí cuenta el escaneo recién insertado al evaluar la
  // multiplicidad. Antes esta lectura ocurría fuera de la transacción y
  // ambos requests podían leer el mismo conteo a la vez, contando dos
  // veces una caja que debía rechazarse la segunda vez.
  ...
  const { escaneo, novedad, cantidadEscaneada, cantidadEsperada, resultado } = await prisma.$transaction(async (tx) => {
    ...
    // La clasificación solo necesita dos números (no el arreglo completo de
    // escaneos, que crecía linealmente dentro del lock): cuántos escaneos
    // VALIDO tiene ESTE código (consulta puntual cubierta por el índice
    // [cargueId, resultado] / [cargueId, codigoEscaneado] — se compara con
    // la multiplicidad registrada del código) y el total de VALIDOS
    // (cantidadEscaneada, que el cargue ya mantiene).
    const escaneosValidosDelCodigo = await tx.gourmetCargueEscaneo.count({
    ...
    const clasificacion = clasificarEscaneoGourmet({
      ...
      previos: { escaneosValidosDelCodigo, cantidadValida: cargue.cantidadEscaneada },
      modoCodigo,
    })
