...
export default defineEventHandler(async event => {
  ...
  return prisma.$transaction(async tx => {
    ...
    if (b.accion === 'validar') {
      ...
      // Solo los picking elegidos (por semaforo): no se resurte todo.
      const elegidos = new Set(Array.isArray(b.plus) ? b.plus.map(String) : [])
      if (!elegidos.size) throw createError({ statusCode: 400, statusMessage: 'Selecciona los picking a resurtir' })
      const tareas = p.filas.filter(f => elegidos.has(f.plu)).flatMap(f => f.tareas).sort((a, z) => a.altura.localeCompare(z.altura, undefined, { numeric: true }))
      if (!tareas.length) throw createError({ statusCode: 400, statusMessage: 'Los picking elegidos no tienen cajas por resurtir' })
      ...
      await tx.activityLog.create({ data: { userId: actor.id, action: 'CREATE', module: 'montaje-resurtido', recordId: m.id, details: `${tareas.length} tareas desde capacidad picking (${elegidos.size} picking elegidos), teórico ${p.carga.id}` } })
