...
describe('Equipos y ayudantes',()=>{
  ...
  it('separa participaciones del porcentaje de cierre sin duplicar tareas',()=>{
    // Cuenta a todos los que la tuvieron; el detalle esta en montajeRealizacion.test.ts.
    ...
    expect(r.find((p:any)=>p.id==='b')).toMatchObject({completadas:1,porcentaje:100,participadas:1});expect(r.find((p:any)=>p.id==='a')).toMatchObject({completadas:1,porcentaje:100,participadas:1})
  })
})
