...
import {
  AlertTriangle, Camera, CheckCircle2, RefreshCw, Save, ShieldCheck,
  Truck, XCircle,
} from "lucide-react";
...
export default function PreoperacionalPage() {
  ...
  if (loading || status === "loading") return <SkeletonTable rows={8} cols={4} />;

  if (role !== "TRANSPORTISTA") {
    return <EmptyState icon={<ShieldCheck size={24} />} title="Sin acceso" description="El preoperacional esta disponible solo para transportistas." />;
  }
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: 980 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 18, marginBottom: 22 }}>
        <Stat value={data.inspeccionHoy ? "Si" : "No"} label="Registrado hoy" color={data.inspeccionHoy ? "var(--success)" : "var(--warning)"} />
      ...
      {data.inspeccionHoy && (
        <div style={{ padding: "12px 14px", border: `1px solid ${ESTADO_COLOR[data.inspeccionHoy.estado]}44`, background: ESTADO_COLOR[data.inspeccionHoy.estado] + "12", borderRadius: 8, marginBottom: 18, display: "flex", alignItems: "center", gap: 10 }}>
      ...
      {toast && (
        <div className="animate-fade-up" style={{ position: "fixed", bottom: 24, right: 24, zIndex: 10000, background: toast.err ? "var(--error)" : "#0F0F10", color: "#fff", padding: "10px 16px", borderRadius: 10, fontSize: 13, fontWeight: 500, boxShadow: "var(--shadow-xl)" }}>
          {toast.msg}
        </div>
      )}
    </div>
