...
// ExcelJS, sin Next — solo transforma filas en tablas. Así se puede testear a pelo
// y el armado del libro (reporteWorkbook.ts) se queda con la parte de formato.
//
...
//
// NOTA: portado 1:1 en nuxt-app/server/utils/exportacionesReporteCalc.ts.
...
export function tiemposPorUsuario(registros: RegistroReporte[]): FilaUsuario[] {
  ...
  return filas.sort(
    (a, b) => b.porAmbito.general.cajas - a.porAmbito.general.cajas || a.nombre.localeCompare(b.nombre, "es"),
  );
}
...
export function matricesDiarias(
  ...
  const usuarios = ordenarPorNombre(
    Array.from(
      new Map(registros.map((r) => [r.creadoPorId, { id: r.creadoPorId, nombre: r.creadoPorNombre }])).values(),
    ),
  );
