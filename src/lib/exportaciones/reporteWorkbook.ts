...
//
// Reparto de responsabilidades: reporteCalc.ts calcula, este archivo coloca celdas
// y declara dónde va cada gráfica, y excelCharts.ts inyecta el XML. Las refs de las
// gráficas se construyen con los mismos índices con los que se escriben las celdas
// (nada de rangos "a ojo"), así que mover un bloque no descuadra sus series.
//
// NOTA: portado 1:1 en nuxt-app/server/utils/exportacionesReporteWorkbook.ts.
// Mantener ambos en sync.
import ExcelJS from "exceljs";
import { workbookBuffer } from "@/lib/excel";
import { cellRef, injectCharts, rangeRef, type ChartSpec } from "@/lib/excelCharts";
...
export async function construirReporte(registros: RegistroReporte[]): Promise<Buffer> {
  ...
  return injectCharts(await workbookBuffer(wb), specs);
}
