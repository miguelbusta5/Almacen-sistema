// ═══════════════════════════════════════════════════════════
// CLASIFICACIÓN DE ESCANEO — CARGUE GOURMET
// Función pura, sin acceso a base de datos — el endpoint (G3B)
// le pasa los datos ya consultados y aplica el resultado.
// ═══════════════════════════════════════════════════════════
...
export function clasificarEscaneoGourmet(
  input: ClasificarEscaneoGourmetInput
): ClasificarEscaneoGourmetResultado {
  ...
  if (!formatoValido(codigo)) {
    return {
      ...
      debeCrearNovedad: false,
      incrementaContador: false,
      mensaje: "Código vacío o ilegible — vuelve a escanear.",
    };
  }
  ...
  if (modoCodigo === "QR_UNICO_CAJA") {
    ...
    if (!perteneceAlPedido) {
      return {
        resultado: "CAJA_AJENA",
        debeCrearNovedad: true,
        tipoNovedadSugerido: "CAJA_AJENA",
        incrementaContador: false,
        mensaje: "Esta caja no pertenece a este pedido.",
      };
    }
    if (previosNorm.includes(codigoNorm)) {
      return {
        resultado: "DUPLICADO",
        debeCrearNovedad: true,
        tipoNovedadSugerido: "CAJA_DUPLICADA",
        incrementaContador: false,
        mensaje: "Esta caja ya fue escaneada antes en este cargue.",
      };
    }
    return {
      resultado: "VALIDO",
  ...
  if (modoCodigo === "QR_SOLO_ORDEN") {
    if (!perteneceAOrden(codigoNorm, ordenPedido)) {
      return {
        ...
        tipoNovedadSugerido: "CAJA_AJENA",
        incrementaContador: false,
        mensaje: "El código no corresponde a la orden de este pedido.",
      };
    }
    if (cantidadValidaPrevia >= cajasEsperadas) {
      return {
        ...
        debeCrearNovedad: true,
        tipoNovedadSugerido: "DIFERENCIA_CANTIDAD",
        incrementaContador: false,
        mensaje: "Ya se alcanzó la cantidad esperada de cajas para esta orden.",
      };
    }
    return {
      resultado: "VALIDO",
      debeCrearNovedad: false,
      incrementaContador: true,
      mensaje: "Escaneo válido (código de orden, no certifica caja física única).",
    };
  }
